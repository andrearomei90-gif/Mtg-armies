...
function buildPrompt(mode, payload) {
  const rules = `Formato Commander Armies. Il mazzo deve avere 100 carte incluso il Commander, identità colore Commander, singleton salvo terre base o eccezioni testuali. Le carte devono rispettare Tribe e Key secondo i filtri dell'app. Le terre pure sono sempre valide; le terre con altri tipi devono superare i normali filtri. Le creature che scelgono un tipo di creatura e diventano quel tipo sono valide; il solo Cangiante non basta.`;
...
function schemaFor(mode) {
  if (mode === 'suggest') return {
    ...
    properties: {
      commander: { type: 'string' }, tribe: { type: 'string' }, key: { type: 'string' },
      score: { type: 'integer', minimum: 0, maximum: 100 }, strategy: { type: 'string' }, reason: { type: 'string' }
    }
  };
  if (mode === 'design') return {
    type: 'object', additionalProperties: false,
    required: ['name', 'summary', 'targets', 'core_cards'],
    properties: {
